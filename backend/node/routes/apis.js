...
const NftContract = new web3.eth.Contract(
  contractABI.abi,
  ContractDetails.nftContractAddress
); //loadContract();
  ...
  try {
    ...
    if (token.startsWith("ipfs://")) {
      ...
      resp = await axios.get(
        `https://cf-ipfs.com/ipfs/${token.split("//")[1]}`
      ...
      resp = await axios.get(
        `https://gateway.ipfs.io/ipfs/${token.split("ipfs/")[1]}`
      );
    }
...
router.get("/asset/:contract/:token", async (req, res) => {
  ...
  try {
    if (filter == "nft_meta") {
      ...
      try {
        if (
          contractAddr.toLocaleLowerCase() ==
          ContractDetails.nftContractAddress.toLocaleLowerCase()
        ) {
      ...
      if (ContractDetails.list1155contracts.includes(contractAddr)) {
        [response, history, mongoresp] = await Promise.all([p2, p4, p5]);
      } else {
      ...
      data.chain = "Mumbai Polygon Testnet";
      ...
      if (
        contractAddr.toLowerCase() == ContractDetails.nftContractAddress ||
        ContractDetails.list1155contracts.includes(contractAddr)
      ) {
        // On Sale
...
router.post("/pre-mint", auth, async (req, res) => {
  ...
  if (multi != true) {
    ...
    try {
      let data = contract1.methods
        .mintNFT(`ipfs://${json_hash.Hash}`)
        .encodeABI();
