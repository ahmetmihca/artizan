...
const get_nfts = async (id) => {
    const response = await axios.get(`http://localhost:3001/api/v1/listNFT/owner/${id}` , { headers: headers });
    return response.data;
};


const get_user_favorites = async (user_id) => {
    const response = await axios.get(BASE_URL + `/favorites/${user_id}`, { headers: headers });
...
const get_history = async (id, page) => {
    const response = await axios.get(`http://localhost:3001/api/v1/listTransaction/user/${id}/${page}`, { headers: headers });
    return response.data
}

const get_user_created_nfts = async (id) => {
    const response = await axios.get(`http://localhost:3001/api/v1/user/created/${id}` , { headers: headers });
    return response.data;
};


const user_services = {
    update_user, get_user, get_my_watchlist, removeWatchlist, addFavoriteNft, get_user_favorites, removeFavoriteNft, get_nfts, addWatchlist, get_my_favorites, get_history, get_user_created_nfts
}
