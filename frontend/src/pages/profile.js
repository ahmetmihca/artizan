...
function Profile(props) {
    ...
    const fetch_user_created_nfts = async () => {
        ...
        await Promise.all(nfts.createdNFTs.map(async (element) => {
            let asset = await asset_services.get_asset(element.contract, element.tokenID, "meta");
            let a = { "asset": asset, "token": "0x" + element.tokenID.toString(16), "contract": element.contract }
            if(a.asset && a.asset.name && a.asset.imgURL){
                ...
                <MDBRow>
                    ...
                    <MDBCol size='3' >
                        ...
                        <ProfileCard
                            ...
                            avatar={'http://localhost:3001/public/users/' + user.id + "_avatar.png"}
                            bio={user.bio}
                            wallet={user.id}
                        />
                    ...
                    <MDBCol>
                        ...
                        <MDBTabsContent>
                            ...
                            <MDBTabsPane show={fillActive === 'tab2'}>
                                <MDBRow>
                                    {created_nfts ? created_nfts.filter(f => f.asset.name.toLowerCase().includes(filter) || filter === '').map((object, i) =>
                                        <ProductCard className="nft-card" img={object.asset.imgURL} name={object.asset.name} token={object.token} address={object.contract} 
                                        favorited={object.asset.favorited}
                                        />
                                    ) : '..'}
                            ...
                            <MDBTabsPane show={fillActive === 'tab3'}>
                                <MDBRow>
                                    {favorite_nfts ? favorite_nfts.filter(f => f.asset.name.toLowerCase().includes(filter) || filter === '').map((object, i) =>
                                        <ProductCard className="nft-card" img={object.asset.imgURL} name={object.asset.name} token={object.token} address={object.contract} 
                                        favorited={object.asset.favorited}
                                        />
                                    ) : '..'}
                            ...
                            <MDBTabsPane show={fillActive === 'tab4'}>
                                <MDBRow>
                                    { userHistory ? <UserActivity user_id={id}></UserActivity>
                                     : '..'}
                                </MDBRow>
                            </MDBTabsPane>
