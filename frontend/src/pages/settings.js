...
function Settings() {
    ...
    let handleSubmit = async (event) => {
        ...
        console.log('token');
        let res = await user_services.update_user(avatar, display_name, username, bio, token);
    ...
    {   
        ...
        currency.change_rate(event.target.options[selectedIndex].getAttribute('data-key'));
    ...
    return (
        <main>
            ...
            <MDBContainer className='mt-3'>
                ...
                <p>Conversion Rate</p>
                <select className='form-select' onChange={changeConversionRate} defaultChecked={ () =>{
                    currency.get_current_rate()
                }}> 
                    <option key={'matic'} data-key={'matic'}>MATIC</option>
                    <option key={'try'} data-key={'try'}>Turkish Lira</option> 
                    <option key={'usd'} data-key={'usd'}>American Dolar</option>   
                </select>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <MDBInput wrapperClass='mb-4' textarea id='form4Example3' label='Wallet ID' disabled='true' value={walletId} />
                    <MDBInput id='form4Example1' wrapperClass='mb-4' label='Display Name' />
