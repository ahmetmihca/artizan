import { MDBBtn, MDBContainer, MDBNavbar, MDBNavbarBrand, MDBNavbarItem, MDBNavbarLink, MDBNavbarNav, MDBIcon, MDBRow, MDBCol } from 'mdb-react-ui-kit';
    ...
    <MDBNavbar expand='lg' className='navbar' >
      <MDBContainer fluid className='w-100'>
        ...
        <form className='input-group ms-4' onSubmit={searchHandler}>
          ...
          <select style={{maxWidth: 160, textAlign: 'center'}} className='form-control-custom' name="category" id="category">
            <option value="collection">Collections 📚 </option>
            <option value="user">Users 🤠 </option>
            <option value="asset">Assets 🧿 </option>
          </select>

          <input type="search" className='form-control-custom px-2 search-bar' placeholder='Search items, collections and accounts' />
          <MDBBtn className='search-btn'>
            <MDBIcon fa icon="search py-0" size='lg' />
          </MDBBtn>
        ...
        <MDBNavbarNav className='justify-content-end mx-5 my-1'>
          <MDBNavbarItem className='dropdown' >
            <MDBNavbarLink href="/explore" aria-current="page" className='dropbtn'>🔎 Explore</MDBNavbarLink>
            <DropdownList dropdowntype="Explore" ></DropdownList>
          </MDBNavbarItem>
          <MDBNavbarItem className='dropdown'>
            <MDBNavbarLink href="/stats" aria-current="page" className='dropbtn'>📈 Stats</MDBNavbarLink>
            <DropdownList dropdowntype="Stats"></DropdownList>
          </MDBNavbarItem>

          <MDBNavbarItem className='dropdown'>
            <MDBNavbarLink href="/my_nfts" aria-current="page">🎒 My Items</MDBNavbarLink>
          </MDBNavbarItem>
          

          <MDBNavbarItem>
            <MDBNavbarLink href="/create" aria-current="page" >📐 Create</MDBNavbarLink>
          </MDBNavbarItem>
          <MDBNavbarItem className='dropdown'>
          ...
          <MDBNavbarItem className='.bg-transparent'>
            <MDBBtn aria-current="page" className='px-3' onClick={() => {login_services.login_system()}} >
              {walletConnected ? (
                <span><MDBIcon fas icon="wallet" size='lg' />  {metamask_operations.getWalletId() ? metamask_operations.getWalletId().substring(0, 4) + "..." + metamask_operations.getWalletId().substring(metamask_operations.getWalletId().length - 4, metamask_operations.getWalletId().length) : ""}</span>
              ) : (
